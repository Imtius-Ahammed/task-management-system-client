...
const CompletedTask = () => {
  ...
  useEffect(() => {
    fetch(
      `https://task-management-system-server.vercel.app/daily/taskCompleted`
    )
      .then((res) => res.json())
      .then((data) => {
        setDailyTasks(data);
  ...
  const handleDeleteTask = (task) => {
    ...
    fetch(
      `https://task-management-system-server.vercel.app/dailyTasks/${task._id}`,
      {
        method: "DELETE",
      }
    )
    ...
    <div>
      <div className="container mx-auto my-10  grid lg:grid-cols-2 grid-cols-1 lg:h-96">
        {dailyTasks.map((tasks) => (
          <Card className="m-2 shadow-xl shadow-orange-300" key={tasks._id}>
